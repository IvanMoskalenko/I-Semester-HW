...
module hw4 =
    ...
    let writeArray file array =
        System.IO.File.WriteAllLines((file), arrayOfIntIntoArrayOfStrings array)

    let writeList file list =
        System.IO.File.WriteAllLines((file), arrayOfIntIntoArrayOfStrings (List.toArray list))
    ...
    let listBubbleSort (list: list<int>) =
        ...
        let rec _go (list: list<int>) =
            let res =
                match list with
                | [] -> []
                | x :: y :: tail -> if x > y then y :: (_go (x :: tail)) else x :: (_go (y :: tail))
                | x :: tail -> [ x ]
    ...
    let listQuickSort list =
        let split opCompare list =
            let rec go lst part1 part2 =
                match lst with
                | [] -> part1, part2
                | hd :: tl -> if opCompare hd then go tl (hd :: part1) part2 else go tl part1 (hd :: part2)
    ...
    let rec quickSortArrayForExperiments =
        function
        | [||] -> [||]
        | x when x.Length < 2 -> x
        | x ->
            let left, (right, pivot) =
                Array.partition (fun i -> i < x.[0]) x
                |> (fun (left, right) -> left, right |> Array.partition (fun n -> n <> x.[0]))

            Array.append
                (Array.append (left |> quickSortArrayForExperiments) pivot)
                (right |> quickSortArrayForExperiments)

    let arrayQuickSortForExperiments (x: int array) = quickSortArrayForExperiments x
